...
let currentQuestionIndex = 0,
  ...
  correctCount = 0,
  wrongCount = 0;
// === পরিবর্তন: সাউন্ড ফাইলের পাথ ঠিক করা হয়েছে ===
...
function showFinalResult() {
  ...
  saveScoreToFirebase(correctCount);
  ...
  container.innerHTML = `
    <div class="text-center space-y-5">
      ...
      <div class="flex flex-wrap justify-center gap-3">
        ...
        <button onclick="window.location.href='../user-dashboard.html'" class="action-btn">ড্যাশবোর্ড দেখুন</button>
...
// ================================================================
// === নতুন এবং উন্নত Firebase স্কোর সেভিং ফাংশন (সংস্করণ ৪.০) ===
// ================================================================

async function saveScoreToFirebase(finalScore) {
    if (typeof firebase === 'undefined' || !firebase.auth) {
        // console.warn("Firebase is not initialized."); // Optional logging
        return;
    }

    const auth = firebase.auth();
    // This listener will check the user's login state.
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
        unsubscribe(); // Stop listening after the first check to avoid multiple saves.

        if (user) {
            // User is logged in, proceed to save the score.
            ...
            try {
                ...
                return; // Stop if there's an error checking the role.
            }
            ...
            const wantsToLogin = confirm("স্কোর সেভ করার জন্য আপনাকে লগইন করতে হবে। আপনি কি এখন লগইন করতে চান?");
            if (wantsToLogin) {
                window.location.href = '../login.html'; // Redirect to the main site's login page.
            }
        }
    });
}
